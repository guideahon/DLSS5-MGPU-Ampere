...
HOST_PRIMARY_PCI="${MGPU_NGX_PRIMARY_PCI:-}"
...
CUDA_BRIDGE_SOURCE_ORDINAL="${MGPU_CUDA_BRIDGE_SOURCE_ORDINAL:-}"
CUDA_BRIDGE_DESTINATION_ORDINAL="${MGPU_CUDA_BRIDGE_DESTINATION_ORDINAL:-}"
...
EXPORT_RESOURCE_FD="${VKD3D_EXPORT_RESOURCE_FD:-}"
CUDA_HELPER_LOG="${MGPU_CUDA_HELPER_LOG:-}"
HOST_LD_PRELOAD="${MGPU_OFFICIAL_HOST_LD_PRELOAD:-}"

# Resource allocations must carry VkExportMemoryAllocateInfo at creation time;
# setting the flag only inside the bridge would be too late for game resources.
if [[ "${DLSSNR_TRANSPORT}" == "resource-fd-probe" ]]; then
  EXPORT_RESOURCE_FD=1
fi
...
setsid bash -c 'cd "$1"; shift; exec "$@"' bash "${HOST_TMP}" env \
  ...
  MGPU_NGX_PRIMARY_PCI="${HOST_PRIMARY_PCI}" \
  ...
  MGPU_CUDA_BRIDGE_SOURCE_ORDINAL="${CUDA_BRIDGE_SOURCE_ORDINAL}" \
  MGPU_CUDA_BRIDGE_DESTINATION_ORDINAL="${CUDA_BRIDGE_DESTINATION_ORDINAL}" \
  ...
  VKD3D_EXPORT_RESOURCE_FD="${EXPORT_RESOURCE_FD}" \
