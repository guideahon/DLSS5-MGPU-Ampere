...
static int run_fenced_persistent(int argc, char **argv)
{
    ...
    const int pair_count = std::atoi(argv[5]);
    const char *status_log = argv[6];
    const char *command_path = argv[7];
    if (fence_count < 1 || fence_count > 16) return 2;
    if (pair_count < 1 || pair_count > 8) return 2;
    const int resource_base = 8 + fence_count * 2;
    if (argc != resource_base + pair_count * 8) return 2;
    ...
    for (int index = 0; index < pair_count && result == CUDA_SUCCESS; ++index) {
    ...
    for (int index = 0; index < fence_count && result == CUDA_SUCCESS; ++index) {
        const int wait_fd = std::atoi(argv[8 + index * 2]);
        const int signal_fd = std::atoi(argv[9 + index * 2]);
