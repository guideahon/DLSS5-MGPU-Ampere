...
struct VisualStats {
    unsigned char min_u8 = 255;
    unsigned char max_u8 = 0;
...
static VisualStats inspect_rgba8(const unsigned char* data, UINT width,
    ...
    for (UINT y = 0; y < height; ++y) {
        ...
        for (UINT x = 0; x < width; ++x) {
            ...
            stats.min_u8 = std::min(stats.min_u8, std::min(red, std::min(green, blue)));
            stats.max_u8 = std::max(stats.max_u8, std::max(red, std::max(green, blue)));
            if (red || green || blue) ++stats.nonzero_pixels;
        }
    }
    return stats;
}

static VisualStats inspect_rgba16f(const unsigned char* data, UINT width,
                                   UINT height, UINT row_pitch) {
    VisualStats stats;
    for (UINT y = 0; y < height; ++y) {
        const unsigned char* row = data + static_cast<size_t>(y) * row_pitch;
        for (UINT x = 0; x < width; ++x) {
            ...
            const unsigned char red = float_to_u8(half_to_float(
                static_cast<uint16_t>(pixel[0] | pixel[1] << 8)));
            const unsigned char green = float_to_u8(half_to_float(
                static_cast<uint16_t>(pixel[2] | pixel[3] << 8)));
            const unsigned char blue = float_to_u8(half_to_float(
                static_cast<uint16_t>(pixel[4] | pixel[5] << 8)));
            stats.min_u8 = std::min(stats.min_u8, std::min(red, std::min(green, blue)));
            stats.max_u8 = std::max(stats.max_u8, std::max(red, std::max(green, blue)));
            if (red || green || blue) ++stats.nonzero_pixels;
        }
...
static bool write_rgba16f_ppm(const char* path, const unsigned char* data,
                              UINT width, UINT height, UINT row_pitch) {
    if (!path || !*path || !data || !width || !height || row_pitch < width * 8)
        return false;
    std::ofstream file(path, std::ios::binary | std::ios::trunc);
    if (!file) return false;
    file << "P6\n" << width << " " << height << "\n255\n";
    for (UINT y = 0; y < height; ++y) {
        ...
        for (UINT x = 0; x < width; ++x) {
            const unsigned char* pixel = row + x * 8;
            const uint16_t r = static_cast<uint16_t>(pixel[0] | pixel[1] << 8);
            const uint16_t g = static_cast<uint16_t>(pixel[2] | pixel[3] << 8);
            const uint16_t b = static_cast<uint16_t>(pixel[4] | pixel[5] << 8);
            const unsigned char rgb[3] = {
                float_to_u8(half_to_float(r)), float_to_u8(half_to_float(g)),
                float_to_u8(half_to_float(b))};
            file.write(reinterpret_cast<const char*>(rgb), sizeof(rgb));
        }
    }
    return static_cast<bool>(file);
...
int main() {
    ...
    if (SUCCEEDED(hr) && mapped) {
        ...
        const char* input_capture_path = std::getenv("MGPU_CAPTURE_INPUT_PPM_PATH");
        if (input_capture_path && *input_capture_path)
            std::fprintf(stderr, "cross_adapter_capture_input_ppm path=%s written=%s\n",
                         input_capture_path,
                         write_rgba16f_ppm(input_capture_path, readback_bytes,
    ...
    VisualStats ngx_visual_stats;
    bool ngx_visual_valid = false;
    UINT64 ngx_fnv1a = 0;
    ...
    if (ngx_requested && ngx_readback) {
        ...
        if (SUCCEEDED(ngx_map) && ngx_mapped) {
            ...
            ngx_visual_stats = presentation_requested
                ? inspect_rgba8(ngx_bytes_ptr, 1280, 720,
            ...
            if (capture_path && *capture_path) {
                capture_ppm_written = presentation_requested
                    ? write_rgba8_ppm(capture_path, ngx_bytes_ptr, 1280, 720,
                                      ngx_footprint.Footprint.RowPitch)
                    : write_rgba16f_ppm(capture_path, ngx_bytes_ptr, 1280, 720,
                                        ngx_footprint.Footprint.RowPitch);
            }
        ...
        std::fprintf(stderr, "cross_adapter_ngx init=0x%08x create=0x%08x evaluate=0x%08x readback_map=0x%08lx nonzero=%llu fnv1a=0x%016llx visual_min=%u visual_max=%u visual_nonzero_pixels=%llu visual_validation=%s validation=%s\n",
                     ...
                     static_cast<unsigned int>(ngx_visual_stats.min_u8),
                     static_cast<unsigned int>(ngx_visual_stats.max_u8),
                     static_cast<unsigned long long>(ngx_visual_stats.nonzero_pixels),
                     ngx_visual_valid ? "ok" : "FAIL",
